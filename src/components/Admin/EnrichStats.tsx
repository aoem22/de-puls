...
/* ── Component ──────────────────────────────────────────────── */
export function EnrichStats({ selectedYear, selectedBl, activeDayData, byDay, byBundesland }: EnrichStatsProps) {
  void byDay;
  void byBundesland;
      ...
      <div>
        ...
        {flagged.length > 0 && (
          <p className="mt-1.5 text-[10px] font-medium" style={{ color: '#ef4444' }}>
            L&uuml;cken: {flagged.map((i) => MONTH_NAMES[i]).join(', ')}
          </p>
        )}
      </div>
    </div>
