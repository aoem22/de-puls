...
interface Props {
  ...
  onBlChange: (bl: string | null) => void;
}
...
const COLOR_LOW = 'rgba(34, 197, 94, 0.3)';
const COLOR_MED = 'rgba(34, 197, 94, 0.6)';
const COLOR_HIGH = 'rgba(34, 197, 94, 1.0)';
...
export function ScrapeHeatmap({ byDay, byBundesland, selectedYear, onYearChange, selectedBl, onBlChange }: Props) {
  ...
  // Pick the right day data based on selection
  ...
  return (
    <div>
        ...
        <div className="flex gap-1">
          {years.map((y) => (
            <button
              ...
              style={{
                borderColor: y === selectedYear ? 'var(--accent)' : 'var(--border)',
      ...
      <div className="mb-3 flex flex-wrap gap-1">
        <button
          onClick={() => onBlChange(null)}
          className="rounded-lg border px-2 py-0.5 text-[10px] font-semibold uppercase transition-colors"
          style={{
            borderColor: selectedBl === null ? 'var(--accent)' : 'var(--border)',
            background: selectedBl === null ? 'var(--accent)' : 'transparent',
