-- Add suspect_solo_count to dashboard_context_stats RPC
-- Counts records where suspect_count = 1 (Einzeltäter metric)
-- Also restores p_bundesland parameter
...
BEGIN
  ...
  -- Suspect and Victim counts + solo count
  SELECT
    COALESCE(SUM(cr.suspect_count), 0),
    COUNT(cr.suspect_count),
    COUNT(cr.suspect_count) FILTER (WHERE cr.suspect_count = 1),
    COALESCE(SUM(cr.victim_count), 0),
    COUNT(cr.victim_count)
  INTO v_suspect_count_sum, v_suspect_count_cases, v_suspect_solo_count, v_victim_count_sum, v_victim_count_cases
  FROM crime_records cr
  WHERE cr.hidden = false
    AND (cr.classification IN ('crime', 'update') OR cr.classification IS NULL)
    AND cr.published_at >= p_start AND cr.published_at < p_end
  ...
  RETURN jsonb_build_object(
    ...
    'suspect_solo_count', v_suspect_solo_count,
